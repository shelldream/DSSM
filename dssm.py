...
class DSSM(object):
    def __init__(self, n_input, n_l1_hidden, n_l2_hidden, \
            transfer_function=tf.nn.relu, optimizer=tf.train.AdamOptimizer, is_dropout=True, drop_rate=0.5)
        ...
        self.query_batch = tf.sparse_placeholder(tf.float32, [None, self.n_input], name="QueryBatch")
        self.pos_doc_batch = tf.sparse_placeholder(tf.float32, [None, self.n_input], name="PosDocBatch")
        self.neg_doc_batch = tf.sparse_placeholder(tf.float32, [None, self.n_input], name="NegDocBatch")
        self.weight = self._initialize_weights()
        
        # loss function
        self.query_output = self._model_output(self.query_batch)
        self.pos_doc_output = self._model_output(self.pos_doc_batch)
        self.neg_doc_output = self._model_output(self.neg_doc_batch)
    ...
    def _model_output(self, data_batch):
        """构建模型""" 
